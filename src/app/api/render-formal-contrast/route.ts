...
const blockSchema = z.object({
  informalGerman: z.string().min(1),
  informalVi: z.string().min(1),
  formalGerman: z.string().min(1),
  formalVi: z.string().min(1),
...
const runStill = (outputPath: string, propsPath: string) =>
  new Promise<string>((resolve, reject) => {
    const child = spawn('npx', ['remotion', 'still', 'src/index.ts', 'FormalContrastImage', outputPath, '--props', propsPath], {
      cwd: rootDir,
      env: process.env,
    });
    let log = '';
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error(log || `Formal contrast render failed with exit code ${code}`));
      }
    });
  });
...
export const POST = async (request: Request) => {
  ...
  try {
    ...
    for (const [index, block] of input.blocks.entries()) {
      const props = {...block, backgroundUrl, watermarkUrl};
      const propsPath = join(jobDir, 'props', `formal-contrast-${index + 1}.json`);
      const outputFileName = `formal-contrast-${String(index + 1).padStart(2, '0')}-${slugify(block.formalGerman)}-${jobId}.png`;
      const outputPath = join(outDir, outputFileName);

      await writeFile(propsPath, JSON.stringify(props, null, 2));
      logs.push(`[${block.formalGerman}] Rendering formal contrast image...`);
    ...
    logs.push(error instanceof Error ? error.message : 'Formal contrast render failed');
    return NextResponse.json(
      {ok: false, message: error instanceof Error ? error.message : 'Formal contrast render failed', logs},
      {status: 400},
    );
  }
};
